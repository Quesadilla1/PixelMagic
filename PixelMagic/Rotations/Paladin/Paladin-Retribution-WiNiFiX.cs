...
namespace PixelMagic.Rotation
{
    public class PaladinRetriWinifix : CombatRoutine
    {
        ...
        public override void Pulse()
        {
            ...
            if (WoW.CanCast("Judgment") && WoW.CurrentHolyPower >= 5)
            {
                WoW.CastSpell("Judgment");
                return;
            }

            if (WoW.CurrentHolyPower == 0 && WoW.CanCast("Wake of Ashes"))
            {
                WoW.CastSpell("Wake of Ashes");
                return;
            }

            if (WoW.CanCast("Crusade") && WoW.CurrentHolyPower >= 5 && WoW.TargetHasDebuff("Judgment"))
            {
            ...
            if (WoW.CanCast("Execution Sentence") && WoW.CurrentHolyPower >= 3 && WoW.TargetHasDebuff("Judgment") && !WoW.TargetHasDebuff("Execution Sentence"))
            {
                WoW.CastSpell("Execution Sentence");
                return;
            }

            if (WoW.CanCast("Templars Verdict") && WoW.CurrentHolyPower >= 3 && WoW.TargetHasDebuff("Judgment"))
            {
                WoW.CastSpell("Templars Verdict");
                return;
            }

            if (WoW.CanCast("Blade of Justice") && WoW.CurrentHolyPower <= 3) // Higher Priority because it can generate 2 holy power in 1 go
            {
                WoW.CastSpell("Blade of Justice");
                return;
            }

            if (WoW.CanCast("Crusader Strike") && WoW.CurrentHolyPower < 5 && WoW.PlayerSpellCharges("Crusader Strike") >= 0)
            {
